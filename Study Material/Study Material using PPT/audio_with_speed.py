...
class AudioService:
    ...
    def generate_voiceover(self, text, audio_file, speed=1.0):
        if text:
    ...
    def get_audio(self, prompts, speed=1.0):
        for i, prompt in enumerate(prompts):
            content = self.generate_slide_content(prompt)
            if content:
                audio_filename = f'audio_mistral_{i}.mp3'
                self.generate_voiceover(content, audio_filename, speed)
...
AudioService().get_audio([prompts], speed=1.2)